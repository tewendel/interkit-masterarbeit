...
export async function load({ params }) {
	...
	/* NOTE: the dynamic imports are only implemented to paths
	 * exactly two levels deep.
	 * this works: /guides/overview/welcome
	 * this doesn't: /guides/welcome
	 */

	// try .svelte
	if (!post) {
		try {
			post = await import(`../../content/${pathParts[0]}/${pathParts[1]}/${pathParts[2]}.svelte`);
			logFound = '.svelte';
		} catch (error) {
			console.error(error);
			logTried += ' .svelte';
	...
	if (!post) {
		try {
			post = await import(
				`../../content/${pathParts[0]}/${pathParts[1]}/${pathParts[2]}.svelte.md`
			);
			logFound = '.svelte.md';
		} catch (error) {
			logTried += ' .svelte.md';
	...
	if (!post) {
		try {
			post = await import(`../../content/${pathParts[0]}/${pathParts[1]}/${pathParts[2]}.svx`);
			logFound = '.svx';
		} catch (error) {
			logTried += ' .svx';
			logError = error;
