...
Data is provided through an element context, for example using [DataLoaderSingle](/components/DataLoader).

The viewer is a full screen overlay and should be displayed inside its own route. The close button invokes the "back" route (see [Routing](/components/Route)).
