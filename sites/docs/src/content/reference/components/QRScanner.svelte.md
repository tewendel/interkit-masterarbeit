...
The scanner is a full screen overlay and should be displayed inside its own route. The close button invokes the "back" route (see [Routing](/reference/components/Route)).

The QRScanner can be used in two different ways:

1. Scan one of many codes

Open a [DataRoute](/reference/components/Route) corresponding to the scanned code. The user is able to scan all codes available in the supplied sheet.
